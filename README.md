...
| [Hugging Face CLI](https://huggingface.co/docs/huggingface_hub/package_reference/environment_variables#hfhubdisabletelemetry) | `DO_NOT_TRACK` / `HF_HUB_DISABLE_TELEMETRY` | `1` / `1` | By default, some data is collected by HF libraries (transformers, datasets, gradio,..) to monitor usage, debug issues and help prioritize features. Each library defines its own policy (i.e. which usage to monitor) but the core implementation happens in huggingface_hub.|
